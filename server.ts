...
import { Product, Order, OrderStatus, CustomerCRM, DiscountCode, StockMovement, ProductVariant, UserAccount } from './src/types';
...
// In-Memory Real State Store (persists during process lifetime)
let products: Product[] = JSON.parse(JSON.stringify(INITIAL_PRODUCTS));
let orders: Order[] = JSON.parse(JSON.stringify(INITIAL_ORDERS));
let customers: CustomerCRM[] = JSON.parse(JSON.stringify(INITIAL_CUSTOMERS));
let discounts: DiscountCode[] = JSON.parse(JSON.stringify(INITIAL_DISCOUNTS));
let movements: StockMovement[] = JSON.parse(JSON.stringify(INITIAL_MOVEMENTS));

// Password Hashing Helper
function hashPassword(password: string): string {
  return crypto.createHash('sha256').update(password + '_zaya_atelier_secret_2026').digest('hex');
}
...
app.post('/api/auth/register', (req, res) => {
  try {
    ...
    const token = `zy_${crypto.randomBytes(28).toString('hex')}`;
...
// Login customer account (by Email OR Phone)
...
// Demo accounts endpoint for instant test login
app.get('/api/auth/demo-accounts', (req, res) => {
  res.json({
    ...
    accounts: [
      ...
      {
        label: 'Sarah Benali (Hydra • Commandes actives)',
        identifier: 'sarah@zaya.dz',
        password: 'sarah2026',
        role: 'Client'
      }
    ]
